...
params:
  ...
  input_files: "/disks/DATA/Projects/PPPeQTL/IMIDIA/Omni2.5/PLINK_140420_1443/PPPeQTL" #NULL
  output_directory: "/disks/DATATMP/SB_lning/qc_plink2" #NULL
  cohort_name: "PPPeQTL" #NULL
  array: "Illumina Omni2.5" #NULL
  ...
  bin_path: !r list(bcftools = "/usr/bin/bcftools", bgzip = "/usr/bin/bgzip", plink = "/disks/PROJECT/SB_lning/git_repo/plink2")
...
* Mendelian error rate threshold:
    ...
    + Maximum per-variant error rate: 
...
snps_number <- nrow(data.table::fread(input = file.path(temp, "data.pvar"), select = 1))
...
freqsnp <- data.table::fread(
  file = file.path(temp, "data.afreq"), 
  colClasses = c("ALT_FREQS" = "numeric", "OBS_CT" = "numeric")
)[, MAF := min(ALT_FREQS, 1 - ALT_FREQS), by = "ID"]
data.table::setnames(freqsnp, old = "#CHROM", new = "CHROM")

data.table::fwrite(
  x = freqsnp[MAF >= params[["maf_threshold"]], .(ID)], 
...
data_maf_lower <- freqsnp[MAF < params[["maf_threshold"]], .(ID)]
...
callrate_samples_geq <- data.table::fread(
  file = file.path(temp, "data_maf_geq.smiss"),
  colClasses = c("#FID" = "character", "IID" = "character")
)[order(F_MISS)]
...
if (is_maf_lower) {
  ...
  callrate_samples_lower <- data.table::fread(
    file = file.path(temp, "data_maf_lower.smiss"),
    colClasses = c("#FID" = "character", "IID" = "character")
  )[order(F_MISS)]
...
sys(paste(plink, 
  "--pfile", file.path(temp, "data"), 
  "--check-sex", min(params[["sex_threshold"]]), max(params[["sex_threshold"]]), # Unrecognized flag
  "--out", file.path(temp, "data")
))
# to manually calculate homozygosity rate on X?
...
heterozygosity <- merge(
  x = data.table::fread(
    ...
    col.names = c("FID", "IID", "O_HOM", "E_HOM", "OBS_CT", "COEF_F"),
    colClasses = c("#FID" = "character", "IID" = "character"), 
    check.names = TRUE
  ), 
...
heterozygosity[, hrate := (OBS_CT - O_HOM) / OBS_CT, by = F_MISS < 1 - params[["callrate_samples"]]]
...
if (is_maf_lower) {
  ...
  heterozygosity_maf_lower[, hrate := (OBS_CT - O_HOM) / OBS_CT, by = F_MISS < 1 - params[["callrate_samples"]]]
...
heterozygosity[!is.na(labs), .(FID, IID, O_HOM, E_HOM, OBS_CT, hrate)] %>% 
  gt::gt(auto_align = "center") %>% 
  gt::tab_header(paste(
...
sys(paste(plink, 
  ...
  "--indep-pairwise 50 1 0.2", # <window size>['kb'] [step size (variant ct)] <unphased-hardcall-r^2 threshold>
  "--make-pgen",
  "--out", file.path(temp, "data_pruned")
))

sys(paste(plink, 
  "--pfile", file.path(temp, "data_pruned"), 
  "--freq", 
  "--out ", file.path(temp, "data_pruned")
))
...
sys(paste(plink,
  "--pfile", file.path(temp, "data_pruned"), 
  "--read-freq", file.path(temp, "data_pruned.afreq"), 
  "--genome", # Unrecognized flag
...
data.table::fwrite(
  x = data.table::fread(file.path(temp, "data.pvar"))[`#CHROM` %in% 1:22, .(ID)],
...
data.table::fwrite(
  x = data.table::fread(file.path(temp, "data_pop.pvar"))[
    !paste(`#CHROM`, POS) %in% unique(paste(`#CHROM`, POS)[duplicated(paste(`#CHROM`, POS))]) &
      !ID %in% unique(ID[duplicated(ID)]), 
    .(ID)
  ],
...
invisible(parallel::mclapply(1:22, mc.cores = min(parallel::detectCores(), 22), function(i) {
  ...
  data.table::fwrite(
    x = data.table::fread(file.path(temp, file.path("1kgenome_pruned", sprintf("1kgenome_pruned_chr%02d_tmp.pvar", i))))[duplicated(ID), .(ID)],
  ...
  sys(paste(plink, 
    "--pfile", file.path(temp, file.path("1kgenome_pruned", sprintf("1kgenome_pruned_chr%02d_tmp", i))),
...
sys(paste(plink, "--pmerge-list", list_1kgenome, "--out", out)) # Segmentation fault (core dumped)
...
if (file.exists(paste0(out, "-merge.missnp"))) {
  ...
  sys(paste(plink, 
    "--pfile", first_chromosome, 
    "--exclude", paste0(out, "-merge.missnp"), 
    "--pmerge-list", list_1kgenome, 
    "--snps-only",
    "--make-pgen", 
    "--out", out
  ))
...
sys(paste(plink, 
  "--pfile", file.path(temp, "1kgenome_pruned", "1kgenome_pruned_complete"),
  "--pmerge", 
